...
## 18. GitHub 업로드 시 주의사항

다음 파일과 폴더는 GitHub에 올리지 않는 것을 권장합니다.
...
특히 `.venv`는 로컬 가상환경 폴더이므로 저장소에 포함하지 않는 것이 좋습니다. 이미 업로드했다면 아래 명령어로 제거할 수 있습니다.

```bash
git rm -r .venv
git commit -m "remove virtual environment folder"
git push origin main
```

그 다음 `.gitignore`에 `.venv/`를 추가하면 다시 업로드되는 것을 방지할 수 있습니다.
